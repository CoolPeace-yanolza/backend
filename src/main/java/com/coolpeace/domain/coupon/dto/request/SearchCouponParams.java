...
public record SearchCouponParams(
        @ValidEnum(enumClass = SearchCouponStatusFilterType.class, message = "올바른 쿠폰 상태 값을 입력해주세요.", required = false)
        String status,
        String title,
        @ValidEnum(enumClass = SearchCouponDateFilterType.class, message = "올바른 쿠폰 날짜 값을 입력해주세요.", required = false)
        String date
) {
}
